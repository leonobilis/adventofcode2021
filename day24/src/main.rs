...
fn initial_population() -> Vec<(i64, Vec<u8>)> {
    ...
    let mut values: Vec<Vec<u8>> = (0..14)
        .map(|_| {
...
fn next_generation(population: &[(i64, Vec<u8>)], sort: &Sort) -> Vec<(i64, Vec<u8>)> {
    let new_populaton: Vec<(i64, Vec<u8>)> = population
        .iter()
        .permutations(2)
        .flat_map(|p| {
            let mut children = Vec::new();

            let child: Vec<u8> = p[0]
                .1
                .iter()
                .take(7)
                .chain(p[1].1.iter().skip(7))
                .copied()
                .collect();
