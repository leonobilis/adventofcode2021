...
fn p1(player1: &u32, player2: &u32) -> u32 {
    ...
    let mut player1 = Player {pos: *player1, score: 0};
    let mut player2 = Player {pos: *player2, score: 0};
    let mut r = 0;

    let mut calc_position = |x: u32| (x + [dice.next(), dice.next(), dice.next()].iter().map(|d| d.unwrap()).sum::<u32>() - 1) % 10 + 1;
    ...
    return player1.score as u32 * r
}


fn p2(player1: &u32, player2: &u32) -> u64 {
    let r_count: HashMap<u8, u8> = [(3, 1), (9, 1),
                                    (4, 3), (8, 3),
                                    (5, 6), (7, 6),
                                    (6, 7) ].iter().cloned().collect();

    let mut cache = HashMap::new();
    let tasks_init: Vec<_> = r_count.keys().map(|roll_sum| (
        Player{pos: *player1, score: 0},
        Player{pos: *player2, score: 0},
        *roll_sum as u8
    )).collect();
    ...
    let (win, lose) = tasks_init.iter()
        .map(|task| *cache.get(task).unwrap())
        .reduce(|acc, cached| (acc.0 + cached.0, acc.1 + cached.1))
        .unwrap();

    if win > lose {win} else {lose}
}
