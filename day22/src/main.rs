...
impl Cube {
    ...
    fn intersection(&self, other: &Cube) -> Option<Cube> {
        let x = (
            max(self.coord().x.0, other.coord().x.0),
            min(self.coord().x.1, other.coord().x.1),
        );
        let y = (
            max(self.coord().y.0, other.coord().y.0),
            min(self.coord().y.1, other.coord().y.1),
        );
        let z = (
            max(self.coord().z.0, other.coord().z.0),
            min(self.coord().z.1, other.coord().z.1),
        );
...
fn p1(cubes: &[Cube]) -> i64 {
    let mut new_cubes = Vec::<Cube>::new();
    cubes
        .iter()
        .take_while(|c| c.coord().x.0.abs() <= 50)
        .for_each(|c| {
            let very_new_cubes: Vec<Cube> = new_cubes
                .iter()
                .filter_map(|nc| nc.intersection(c))
                .collect();
