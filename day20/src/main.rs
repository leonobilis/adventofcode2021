...
impl Image {
    ...
    fn enhance(&self, e_alg: &Vec<bool>) -> Image {
        let start = self.start - 2;
        let end = self.end + 2;
        let mut coords: HashSet::<(i32, i32)> = (start..=end).flat_map(|y| 
            (start..=end).filter_map(move |x| {
                let index: usize = (-1..=1).rev().flat_map(|yy| 
                    (-1..=1).rev().map(move |xx| self.coords.contains(&(x+xx, y+yy)))
                )
                .enumerate()
                .filter(|(_, coord)| *coord)
                .map(|(i, _)| 1 << i)
                .sum();
                if e_alg[index] {Some((x, y))} else {None}
            })
        ).collect();
        if coords.contains(&(start, start)) {
            ...
            coords.extend((start-3..=end+3).flat_map(|a| 
                [
                    (a, start-3), (a, start-2), (a, start-1),
                    (a, end+3), (a, end+2),   (a, end+1),
                    (start-3, a), (start-2, a), (start-1, a),
                    (end+3, a), (end+2, a), (end+1, a)
                ]
            ));
        }
...
fn parse_input(inp: &String) -> (Vec<bool>, Image) {
    let mut splitted = inp.split("\n\n");
    let e_alg = splitted.next().unwrap()
        .chars()
...
fn p1(e_alg: &Vec<bool>, inp_image: &Image) -> usize {
    inp_image.enhance(e_alg).enhance(e_alg).coords.len()
}

fn p2(e_alg: &Vec<bool>, inp_image: &Image) -> usize {
