...
fn parse_input(inp: &str) -> HashMap<(i8, i8), Amphipoda> {
    inp.lines()
        .skip(2)
        .take(2)
        .enumerate()
        ...
        self.amphipods
            ...
            .flat_map(|((x, y), a)| {
                ...
                let spaces: Vec<(i8, i8)> =
                    if *y != 0 && !self.amphipods.contains_key(&(*x, *y - 1)) {
                        (0..*x)
                            .rev()
                ...
                if spaces.contains(&(a.dest, 0)) {
                    let room: Vec<&Amphipoda> = (1..=self.room_depth)
                        .filter_map(|yy| self.amphipods.get(&(a.dest, yy)))
                        .collect();
                    if room.iter().all(|aa| aa.t == a.t) {
                        let new_pos = (a.dest, self.room_depth - room.len() as i8);
                        let new_energy =
                            self.energy + ((x - new_pos.0).abs() + y + new_pos.1) as u32 * a.energy;
                ...
                if *y != 0 {
                    diagrams.extend(
                        spaces
                            .iter()
                            .filter(|(xx, _)| *xx != 2 && *xx != 4 && *xx != 6 && *xx != 8)
                            .filter_map(|(xx, _)| {
                                let new_energy =
                                    self.energy + ((x - xx).abs() + y) as u32 * a.energy;
                                if new_energy >= filter_min {
                                    None
                                } else {
                                    let mut diagram = self.clone();
...
fn solve(amphipods: HashMap<(i8, i8), Amphipoda>, room_depth: i8) -> u32 {
    let mut diagrams = vec![Diagram::new(amphipods, room_depth)];
...
fn p2(amphipods: &HashMap<(i8, i8), Amphipoda>) -> u32 {
    let mut amphipods: HashMap<(i8, i8), Amphipoda> = amphipods
        .iter()
