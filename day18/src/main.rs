...
impl Pair {
    fn reduce(&mut self) {
        while self.explosion_check(0).is_some() {}
        while self.split_check(0).is_some() {}
    }
    ...
    fn explosion_check(&mut self, i: u8) -> Option<(Option<u32>, Option<u32>)> {
        ...
        match check_result {
            CheckResult::ExplodeLeft => {
                let l = self.left.as_pair_ref().unwrap();
                let (lnum, rnum);
                if let (Some(ll), Some(rr)) = (l.left.as_number_ref(), l.right.as_number_ref()) {
                    lnum = *ll;
                    rnum = *rr;
                } else {
                    panic!("check error");
                ...
                Some((Some(lnum), self.right.try_add_left(rnum)))
            }
            CheckResult::ExplodeRight => {
                let r = self.right.as_pair_ref().unwrap();
                let (lnum, rnum);
                if let (Some(ll), Some(rr)) = (r.left.as_number_ref(), r.right.as_number_ref()) {
                    lnum = *ll;
                    rnum = *rr;
                } else {
                    panic!("check error");
                ...
                Some((self.left.try_add_right(lnum), Some(rnum)))
            }
    ...
    fn split_check(&mut self, i: u8) -> Option<(Option<u32>, Option<u32>)> {
        ...
        match check_result {
            CheckResult::SplitExplodeLeft => {
                let lnum = *self.left.as_number_ref().unwrap();
                self.left = PairValue::Number(0);
                Some((
                    Some(lnum / 2),
                    self.right.try_add_left((lnum as f32 / 2.0).ceil() as u32),
                ))
            }
            ...
            CheckResult::SplitExplodeRight => {
                let rnum = *self.right.as_number_ref().unwrap();
                self.right = PairValue::Number(0);
                Some((
                    self.left.try_add_right(rnum / 2),
                    Some((rnum as f32 / 2.0).ceil() as u32),
                ))
            }
...
impl PairValue {
    fn extract_pair(self) -> Option<Pair> {
        match self {
    ...
    fn as_pair_ref(&self) -> Option<&Pair> {
        match self {
            PairValue::Pair(pair) => Some(pair),
            PairValue::Number(_) => None,
        }
    ...
    fn as_number_ref(&self) -> Option<&u32> {
        match self {
    ...
    fn try_add_left(&mut self, value: u32) -> Option<u32> {
        match self {
            ...
            PairValue::Pair(pair) => {
                if pair.left.try_add_left(value).is_none()
                    || pair.right.try_add_left(value).is_none()
                {
                    return None;
                }
                Some(value)
    ...
    fn try_add_right(&mut self, value: u32) -> Option<u32> {
        match self {
            ...
            PairValue::Pair(pair) => {
                if pair.right.try_add_right(value).is_none()
                    || pair.left.try_add_right(value).is_none()
                {
                    return None;
                }
                Some(value)
...
fn parse_input(inp: &str) -> Vec<Pair> {
    ...
    inp.lines()
        .filter_map(|l| parse_pair_value(&mut l.chars()).extract_pair())
        .rev()
        .collect()
}

fn p1(mut numbers: Vec<Pair>) -> u32 {
    let mut add = PairValue::Pair(Box::new(numbers.pop().unwrap()));
    while let Some(number) = numbers.pop() {
        let r = PairValue::Pair(Box::new(number));
        let mut a = Box::new(Pair {
            left: add,
            right: r,
        });
        a.reduce();
...
fn p2(numbers: Vec<Pair>) -> Vec<u32> {
    numbers
        .iter()
        .rev()
        .permutations(2)
        .map(|v| {
            let a = PairValue::Pair(Box::new(v[0].clone()));
            let b = PairValue::Pair(Box::new(v[1].clone()));
